...
void setup() {
  ...
  if (!LoRa.begin(915E6)) {             // initialize ratio at 915 MHz
    Serial.println("LoRa init failed. Check your connections.");
      String message = "Pánic Start";   // send a message
    sendMessage(message);
...
void loop() {
  ...
  if (millis() - lastSendTime >= interval) {
     Serial.println("Intervalo Cumplido, enviando... ");
    String message = "Este es un mensaje desde Lora a 915Mhz";   // send a message
    sendMessage(message);
     digitalWrite(LED_BUILTIN, LOW);   // turn the LED on (HIGH is the voltage level)
